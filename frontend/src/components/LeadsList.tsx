...
export default function LeadsList({ leads, portals, onSelectLead, onAddManualLead, onRefresh, onSwitchTab }: LeadsListProps) {
  ...
  const [selectedTemp, setSelectedTemp] = useState<LeadTemperatura | "ALL">("ALL");
  ...
  const filteredLeads = leads
    .filter((lead) => {
      ...
      const matchStatus = selectedStatus === "ALL" || lead.status_funil === selectedStatus;
      const matchTemp = selectedTemp === "ALL" || lead.temperatura === selectedTemp;
      const matchPortal = selectedPortal === "ALL" || lead.origem_portal === selectedPortal;
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-4 flex flex-col md:flex-row gap-3 items-center justify-between">
        ...
        <div className="relative w-full md:w-80">
          ...
          <input
            ...
            placeholder="Pesquisar por noiva, email, celular ou ID..."
      ...
      <div className="bg-zinc-950 border border-zinc-900 rounded-lg px-4 py-2 text-[10px] font-semibold text-zinc-500 uppercase tracking-wider grid grid-cols-12 gap-4 items-center shrink-0">
          ...
          filteredLeads.map((lead) => {
            ...
            return (
              <button
                ...
                className="w-full bg-zinc-900/60 hover:bg-zinc-900 border border-zinc-800 hover:border-zinc-700 rounded-xl p-4 grid grid-cols-12 gap-4 items-center text-xs text-zinc-300 text-left transition"
              >
                {/* Bride identity */}
                ...
                {/* Email / phone contact */}
                <div className="col-span-3 truncate space-y-0.5">
                ...
                {/* Event Details */}
                <div className="col-span-2 truncate space-y-0.5 pr-2">
                ...
                {/* Date created & arrow */}
                <div className="col-span-1 flex items-center justify-end gap-1 font-mono text-[10px] text-zinc-500">
      ...
      {isAddingLead && (
        <div className="fixed inset-0 bg-black/65 backdrop-blur-sm flex items-center justify-center z-50 p-4 overflow-y-auto">
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl w-full max-w-lg overflow-hidden shadow-2xl flex flex-col max-h-[90vh] md:max-h-[85vh] h-auto my-auto animate-fade-in">
            
            {/* Modal Header */}
            <div className="p-5 border-b border-zinc-800 flex items-center justify-between bg-zinc-950/40 shrink-0">
              <div className="flex items-center gap-2">
                <Plus className="w-5 h-5 text-amber-500" />
                <h3 className="text-base font-semibold text-white">Cadastrar Novo Lead de Noiva</h3>
              </div>
              <button
            ...
            <form onSubmit={handleAddSubmit} className="flex-1 overflow-y-auto p-6 space-y-4 text-xs">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1">
                  <label className="text-zinc-400 font-medium">Nome da Noiva *</label>
                  <input
                    type="text"
                    required
                    value={formNome}
              ...
              <div className="space-y-1">
                ...
                <textarea
                  ...
                  placeholder="Noiva solicitou rótulo personalizado rústico."
              ...
              {/* Action Buttons */}
              <div className="flex justify-end gap-2.5 pt-3 border-t border-zinc-800 shrink-0">
                <button
                  type="button"
                  onClick={() => setIsAddingLead(false)}
